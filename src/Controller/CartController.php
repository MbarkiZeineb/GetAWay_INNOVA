...
class CartController extends AbstractController
{
     ...
     */
    public function index(SessionInterface $s, VoyOrgRepository $rep,Request $requuest,HebergementRepository $reph, VolRepository $repvol, ActiviteRepository $repa): \Symfony\Component\HttpFoundation\Response
    {   $panier = $s->get('panier', []);
        $voyage[] = [];
        $vol[] = [];
        $heb[] = [];
     ...
     *
     */
    public function delete($id, SessionInterface $session, VoyOrgRepository $repv)
    {
        $panier = $session->get('panier', []);
     ...
     *
     */
    public function deleteGroup($idvol,$ida,$idv, SessionInterface $session, VoyOrgRepository $repv)
    {
        $panier = $session->get('panier', []);
     ...
     *
     */
    public function deletetab($id, SessionInterface $session, VoyOrgRepository $repv)
    {
        $panier = $session->get('panier', []);
     ...
     *
     */
    public function check(SessionInterface $s, VoyOrgRepository $rep, HebergementRepository $reph, VolRepository $repvol, ActiviteRepository $repa)
    {$panier = $s->get('panier', []);
        $voyage[] = [];$vol[] = [];$heb[] = [];$act[] = [];
        ...
        if(empty($panier)==false)
        {
            ...
            foreach ($act as $i => $value) {
                if (!empty($value["act"])) {
                    $startDays[] =$value["act"]->getDate()->format('Y-m-d');
                }

            }
            foreach ($voyage as $i => $value) {
                if (!empty($value["voyage"])) {
                    $startDays[] = $value["voyage"]->getDatedepart()->format('Y-m-d');
                }
            }
            $messagesFiltered = [];
            unset($startDays[0]);
     ...
     *
     */
    public function updatecart(Request $request,VoyOrgRepository $repv)
